...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript">
	   var financeCfg = financeCfg || {page:'addFinance'};
	</script>
  </head>
  <body data-attr="addInit">
	 ...
	 <div class="finance">
		<div class="layout">
			...
			<div class="fina-right">
			   <div class="fina-main fina-write fina-pad" id="financeAdd">
				  <form id="financeForm" action="${basePath }<%=response.encodeURL("/user/trade/financial/addFinancial")%>" method="post">
				        <input type="hidden" name="companyNameDigest" id="companyNameDigest" />
				        <input type="hidden" name="financialPhone" id="financialPhone" />
						<h4 class="fina-top" id="autocomplete_finance">
						   <i style="display: none;top:42px;" id="errorMsg"></i>
						   <span><em>*</em>企业名称</span>
						   <input class="fina-in1" type="text" name="companyName" value="" title="" id="companyName" ajaxotherparam="companyName" ajax="${basePath }/company/<%=response.encodeURL("companyNameValidate") %>" ajaxmsg="该企业暂未收录"  minlength="1" des="企业名称" msgtip="right"  placeholder="请填写企业营业执照上的全称" style="color: #444;"/>
						</h4>
						<h4>
						   <span><em>*</em>真实姓名</span>
						   <input class="fina-in2" type="text" name="financialRealname" id="financialRealname" minlength="2" des="真实姓名" msgtip="right" placeholder="请填写真实姓名" style="color: #444;" />
						</h4>
						<h4>
						...
						<h4>
							...
							<input class="fina-in1" type="email" name="financialEmail" id="financialEmail" des="邮箱" minlength="1" msgtip="innerLeft" value=""  placeholder="请填写企业邮箱" style="color: #444;"/>
						</h4>
						<h5><input type="button" id="submitFinance" value="立即申请" style="cursor: pointer;"  /></h5>
				  </form>
				</div>
				
				<div class="fina-main fina-succeed fina-pad" id="financeSuccess" style="display: none;">
					<img src="${basePath}/images/trade-images/fi-icon3.png">
					<h3>恭喜您申请成功！</h3>
					<h4>请保持电话畅通，我们的工作人员稍后会和您取得联系~</h4>
					<h5><span id="timeCountDown">3</span>秒钟后自动跳转信用金融首页</h5>
