require.config({
	...
	paths : {
        ...
        category_operation_state : 'js/uzj/category_operation_state',
		echarts:'js/lib/echarts.min',
		qrcode : 'js/lib/jquery-qrcode-0.14.0',
		popup : 'js/lib/popup_layer',
...
require(['Pingan', 'echarts','Raphael','category_operation_state',"qrcode", 'comment'],function (Pingan,echarts,Raphael, category_operation_state,qrcode){
	...
	Pingan.Base = (function (){
		...
		var turnToLogin = function (){
			window.location.href = Pingan.basePath + "/user/toLogin?from=" + location.href;
		};
		...
		var initCreditLevel = function (){
            $.ajax({
                url:basePath+"/company/score?companyNameDigest="+DetailCfg.companyNameDigest,
...
*/
      ...
      var initDajieScore = function(){
          var companyId = DetailCfg.bizCompanyId;
            var url = basePath+"/company/comment/companydajie/detail?companyId="+companyId;
            $.get(url,function(data){
                if(data && data["data"] && data["data"]["companyScore"] && data["data"]["itemsScore"]){
                    $(".companyScore").show();
                    var aveScore = data["data"]["companyScore"];
                    $("#aveScore").text(aveScore);

                    var companyScore = data["data"];
                    $(".companyScore").find("b").each(function(){
                        var scoreType = $(this).attr("scoreType");
                        var score = parseInt(companyScore[scoreType]);
                        var html = "";
                        html = html +			" <i class=\"star-off\"><i class=\"star-on\" style=\"width: 100%;\">"
                        html = html +				"<label class='istar star-inline' config='{\"n\":" + score + ",\"s\":50,\"size\":12,\"img\":\""+basePath+"/images/star.png\"}'></label>";
                        html = html +			"</i></i>";
                        $(this).before(html);
                        $(this).text(score+"分");
                    });
	...
	Pingan.Load = (function (){
		...
		var loadUserFollow = function(){
				...
				$.post(Pingan.basePath + "/user/favorites/" + DetailCfg.encodExist, {"companyNameDigest" : DetailCfg.companyNameDigest,"companyId":DetailCfg.bizCompanyId}, function(result){
					// $("#follow").html((result.data ? '已' : '') + '关注(' + $("#follow").attr("title") + ')');
                    if(result.statusCode==1){
                    	if(result.data == true)
                    	{
                            $("#follow").html('已关注');
		...
		var loadSearcheHistory = function (){
			...
			$.get(Pingan.basePath+"/company/recentsearched/list",function (result){
				if(result["statusCode"] != 1){
					return;
				}
		...
		var loadCompanyTaxChartsNew = function (){
			...
			companyTaxChartsNew(DetailCfg.companyNameDigest, Pingan.basePath + "/company/tax/list",9);
		    companyTaxChartsNew(DetailCfg.companyNameDigest, Pingan.basePath + "/company/tax/list", 5);
		    companyTaxChartsNew(DetailCfg.companyNameDigest, Pingan.basePath + "/company/tax/list", 6);
		    companyTaxChartsNew(DetailCfg.companyNameDigest, Pingan.basePath + "/company/tax/list", 14);
		    companyTaxChartsNew(DetailCfg.companyNameDigest, Pingan.basePath + "/company/tax/list", 15);
        ...
        //水、电、燃气、公积金缴费趋势
         initWaterElectricGasFundTrend = function (startDateStr,endDateStr,companyNameDigest,isInit){
                $.ajax({
                    url: basePath+"/company/water/electric/gas/fund/trend/ajax/list",
		...
		var initShopSaletrend = function (){
			$.post(Pingan.basePath + "/shopsaletrend/list",{"companyNameDigest" : DetailCfg.companyNameDigest},function (result){
		...
		var initCompanyDomain = function (){
		    ...
		    $.post(Pingan.basePath + "/uwc/detail", "companyId=" + DetailCfg.bizCompanyId, function (resultDm) {
		...
		var initPageHandler = function (){
			...
			changePage.formateItem = function (index, model, row){
				var item = $(model.format(row["changeField"],row["contentBefore"],row["contentAfter"],new Date(row["changeDate"]).format("yyyy-MM-dd")).replace(/null/g, ' '));
            ...
            var initMediaCount = function () {
                $.get(basePath+"/company/ajax/media/count?companyId="+DetailCfg.bizCompanyId,function(data){
                    if(data && data["data"]){
...
/*			var enterPrisePenaltyPage = new Pingan.PageHandler($("#enterPrisePenalty_tbody_list"),$("#enterPrisePenalty").find(".d-page"));
		...
		var initCompanyIntroduce = function () {
            $.ajax({
                url:basePath+"/company/introduce",
	 ...
	 $("#searchForm").submit();
		...
		var initFollow = function (){
			$("#follow").click(function (){
				$.post(Pingan.basePath + "/user/favorites/" + ($(this).hasClass("followed") ? DetailCfg.encodRemove : DetailCfg.encodAdd), {"companyNameDigest" : DetailCfg.companyNameDigest,"companyId":DetailCfg.bizCompanyId} ,function(result){
        ...
        var claimLabel = function () {
            $.ajax({
                type: "GET",
                url: Pingan.basePath + "/company/dbcompany",
...
function uncreDetail(uncreditId){
	$.ajax({
		type: "GET",
		url: Pingan.basePath + "/company/uncre/detail",
...
function courtDetail(id){
	$.post(Pingan.basePath + "/company/court/detail", {"id":id}, function(result) {
...
function annoDetail(id){
	$.post(Pingan.basePath + "/company/courtanno/detail",{"id":id},function(result){
...
function patentDetail(id){
 	$.post(Pingan.basePath + "/company/patent/detail", {"id":id}, function(result) {
...
function punishDetail(id) {
    $.post(Pingan.basePath + "/company/adminpunish/detail", {"id": id}, function (result) {
...
function corpProdAwardDetail(id) {
        $.ajax({
            type: "GET",
            url: Pingan.basePath + "/company/corpProdAward/detail",
            data: {"id":id},
            dataType: "json",
            success: function(data){
                if(data.statusCode==1){
...
function corpProdPenaltyDetail(id){
	$.ajax({
		type: "GET",
		url: Pingan.basePath + "/company/corpProdPenalty/detail",
		data: {"id":id},
		dataType: "json",
		success: function(data){
			if(data.statusCode==1){
...
function enterPrisePenaltyDetail(id){
	$.ajax({
		type: "GET",
		url: Pingan.basePath + "/company/enterPrisePenalty/detail",
		data: {"id":id},
		dataType: "json",
		success: function(data){
			if(data.statusCode==1){
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.statusCode == 1) {
                    ...
                    window.location.href = Pingan.basePath + "/user/toLogin";
                } else {
...
function applyAuthorizationInit(authData) {
    applyAuthorization(DetailCfg.userName, DetailCfg.companyNameDigest, Pingan.basePath + "/user/authorization/" + DetailCfg.encodApply, authData);
}
