...
define('Pingan', ['gov/js/lib/jquery-1.11.3.min'], function (){
	...
	function malert(context,closer,config){
		...
		//关闭
		mask.click(function (){context.fadeOut(time);mask.fadeOut(time);mask.remove();$("html,body").css("overflow","auto");});
		if(closer){
			closer.click(function (){context.fadeOut(time);mask.fadeOut(time);mask.remove();$("html,body").css("overflow","auto");});
		}
	}
            ...
            for(var i = 0; i<types.length; i++){
                ...
                if(type && type != ""){
                    switch (type){
                        ...
                        case 'select' :passed=value!=='';break;
                        default : passed = true;
                    }
                }
                if(passed) break;
	...
	SelectHandler.prototype = {
	    ...
	    "displayMark" : function (){
	    	for(var i=0; i<this.items.length; i++){
	    		...
	    		// console.info(item.attr("data-mark"))
	...
	function PageHandler(content, pContent){
        ...
        this.isShowRedict = cfg.isShowRedict == undefined ? false : cfg.isShowRedict;
	...
	PageHandler.prototype = {
        ...
        "init" : function(){
                this.npage = 1;
                this.urlHandler.params.put("npage", 1);
                this.urlHandler.params.put("total", 0);
                if(this.isAjax){
                    this.loadData();
                }else{
                    this.urlHandler.turn();
