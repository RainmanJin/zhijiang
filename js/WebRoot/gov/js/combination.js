require.config({
	...
	paths : {
		...
		echarts:'gov/js/lib/echarts/build/dist/echarts'
	},
	shim : {
		mCustomScrollbar : {
			deps : [ 'Pingan' ]
  ...
  Pingan.Load = (function (){
	 ...
	 var initCompanySchema = function(cfg){
			...
			var showDetail = function(result) {
				...
				var detailUrl = Pingan.basePath + "/gov/company/professionalInfo?s="+company.companyNo; 
				var schemaUrl = Pingan.basePath + "/gov/company/schema/list?s=" + company.companyNo;
		    ...
		    require.config({
		        paths: {
		            echarts: Pingan.basePath + '/gov/js/lib/echarts/build/dist'
		        }
		    });
		    require( [
			            'echarts',
					    ...
					    if (data.source !== undefined && data.target !== undefined) { //点击的是边
					        ...
					        if(data.category == 1 && data.nodeType == 1) {	//股东	需要根据公司名称去查
					        	...
					        	if(stockName != null && stockName.indexOf("公司") > -1) {
						        	$.post(Pingan.basePath + "/gov/company/schema/detail", {"companyNameDigest":stockName}, function(result) {
					        ...
					        if(data.category==0 || (data.category == 3 && data.nodeType == 1)) {	//对外投资	
					        	$.post(Pingan.basePath + "/gov/company/schema/detail", {"companyNameDigest":data.object.companyNameDigest}, function(result) {
									if(result.statusCode==1) {
										showDetail(result.data);
									}
					        	});
