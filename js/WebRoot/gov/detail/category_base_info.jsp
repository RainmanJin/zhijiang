	...
	<div class="panel-body">
    	<table class="tax-table tax-tabl-top" cellpadding="0" cellspacing="0" border="0">
            ...
            <tr>
                ...
                <td class="sh49"><a href="${basePath}/gov/search?key=${mongoCompany.legalPerson }" >${mongoCompany.legalPerson}</a></td>
            ...
            <tr>
                ...
                <td class="sh49">${mongoCompany.operationStartdate}至${mongoCompany.operationEnddate}</td>
            </tr>
            <tr>
...
<!--主要人员--> 
		...
		<div class="detail-people zstab-block" id="emp_list">
			...
			<c:if test="${not empty empList}" >
				<c:forEach items="${empList}" var="emp">
					<a href="${basePath}/gov/search?key=${emp.employeeName }">
						<div class="base-list">
	...
	<div class="tab-block zsBlock_edit zsBlock_pad ">
        ...
        <c:if test="${!(empty partnerPage.content)}">
                        ...
                        <tr>
                            <td class="sd48">
                                <c:if test="${!(empty partner.companyNo)}">
                                    <a href="${fn:replace(govCompanyDetailUrl,'companyNo',partner.companyNo) }" style="text-decoration: none" partner_name tag_attr="sht">${partner.stockName}</a>
                                </c:if>
                                <c:if test="${empty partner.companyNo}">
                                    <a href="${basePath}/gov/search?key=${partner.stockName}" style="text-decoration: none" partner_name tag_attr="sht">${partner.stockName}</a>
                                </c:if>
                            </td>
            ...
            <div class="d-page" config='{name: "partner", url:"${basePath }/gov/company/partner/list?companyNameDigest=${bizCompany.companyNameDigest}", npage:1, spage:${partnerPage.totalPages }, total:${partnerPage.total }, nodata:"#nodataPartner", pin:2, isAjax:true, autoLoad:false}'></div>
        </c:if>
    </div>
	...
	<%--		<span id="businessInvest_no" class="tab-important">*该项信息需得到企业授权才能查看哦，立即<a href="javascript:void(0);"  onclick="applyAuthorizationInit(7)">申请企业授权</a>
		...
		<div class="d-page" style="display: none;" config='{name: "businessInvestInfo", url:"${basePath }/gov/company/businessInvestInfo/list?companyNameDigest=${bizCompany.companyNameDigest}&authData=7", nodata:"#nodataInvest", pin:2, isAjax:true}'></div>
	...
	<div class="tab-block zsBlock_edit zsBlock_pad ">
		...
		<c:if test="${not empty branchPage && not empty branchPage.content }">
			...
			<div class="d-page" config='{name: "branch", url:"${basePath }/gov/company/branch/list?companyNameDigest=${bizCompany.companyNameDigest}", nodata:"#nodataBranch", npage:1, spage:${branchPage.totalPages }, total:${branchPage.total }, pin:2, isAjax:true, autoLoad:false}'></div>
		...
		<c:if test="${!(empty changepage.content)}">
			<div class="mechanism  zstab-block" id="change_list">
				...
				<c:forEach items="${changepage.content}" var="change">
                    ...
                    <div class="mechanism-list zs_record">
                        <p>变更日期：<span >${change.changeDate}</span></p>
			...
			<div class="d-page" config='{name: "change", url:"${basePath }/gov/company/change/list?companyNameDigest=${bizCompany.companyNameDigest}", nodata:"#nodataChange", npage:${empty param.npage ? '1' : param.npage}, spage:${changepage.totalPages }, total:${changepage.total }, pin:2, isAjax:true, autoLoad:false}'></div>
