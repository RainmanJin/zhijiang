...
<head>
    ...
    <script type="text/javascript" src="${basePath }/gov/js/monitor_new/select2css.js"></script>

    <script>
        var caseCfg = caseCfg || {
                page:'audit',
                clueContent : ${platformCaseInfo.clueContent==null?"":platformCaseInfo.clueContent}
            };
        var clueCfg = clueCfg || { page:''};
    </script>
    ...
    <div class="govList">
        ...
        <div class="govList_gk">
            ...
            <div class="govAudit">
                ...
                <span>${platformCaseInfo.companyName }</span>
            </div>
            <div class="gov_width gov_width3"></div>
        </div>
        <!--end govList_gk -->
        <div class="govList_gk">
            ...
            <div class="govAudit_main">
                ...
                <div class="govAudit_list">
                    ...
                    <div class="govAudit_body">
                        <div class="govAudit_xs_tit">
                            ...
                            <p>创建时间：<span><fmt:formatDate value="${platformCaseInfo.createTime }" pattern="yyyy-MM-dd"/></span></p>
                        </div>
                ...
                <div class="govAudit_list">
                        <form id="caseAuditForm" action="${basePath}/gov/user/case/<%=response.encodeURL("dispose")%>">
                        <h4>处理结果</h4>
                        <input type="hidden" name="id" value="${platformCaseInfo.id}">
                        <div id="uboxstyle" class="govUb">
                            线索状态：
                        <select name="caseProcessType" id="caseProcessType" style="">
                            <option value="" selected='selected'>请选择</option>

                            <%--<option value="English" selected="selected"><font>请选择</font></option>--%>
                            <c:forEach items="${caseProcessTypeMap}" var="caseProcessType">
                                <option value="${caseProcessType.key}">${caseProcessType.value}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div id="uboxstyle01" class="govUb">
                        处理方式：
                        <select name="casePunish" id="casePunish" style="">
                            <option value="" selected='selected'>请选择</option>
                            <c:forEach items="${casePunishTypeMap}" var="casePunish">
                                <option value="${casePunish.key}">${casePunish.value}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div id="uboxstyle01" class="govUb">
                        处理记录：
                        <textarea name="caseVerifyHistory" cols="" rows=""></textarea>
                    </div>
                    <div class="govUb_button govUb_button_left">
                        <a href="javascript:void(0);" id="caseAuditFormBtn" class="govUb_button01">提 交</a>
